...
# Function to load System Prompt from system_prompt.txt file
def load_system_prompt():
    ...
    if os.path.exists(file_path):
        ...
        st.error("⚠️ 'system_prompt.txt' ఫైల్ లభించలేదు! దయచేసి రూట్ ఫోల్డర్‌లో ఫైల్‌ను క్రియేట్ చేయండి.")
        return ""

SYSTEM_PROMPT = load_system_prompt()

# Ticker Mapping for Indian Markets to resolve 'LTP: ₹0.0' error
TICKER_MAP = {
    "NIFTY 50": "^NSEI",
    "NIFTY": "^NSEI",
    "BANK NIFTY": "^NSEBANK",
...
def get_realtime_market_data(symbol_or_name):
    try:
        ...
        # Check dictionary mapping or append .NS for Indian Stocks
        ...
        df = stock.history(period="5d", interval="1m") # Recent intraday data
        
        if df.empty:
            df = stock.history(period="5d") # Fallback to daily data if intraday fails
...
# Sidebar
with st.sidebar:
    st.title("📈 నిధి & సెట్టింగ్స్")
    
    if not api_key:
        api_key = st.text_input("Gemini API Key ఎంటర్ చేయండి:", type="password")
        if api_key:
...
st.markdown("<p class='sub-header'>ICT, Supply & Demand మరియు Live Market Data తో ఆటోమేటిక్ విశ్లేషణ</p>", unsafe_allow_html=True)
...
if user_prompt:
    if not api_key:
        st.error("దయచేసి Gemini API Key ని నమోదు చేయండి.")
    else:
        # Fetch Real-time price for user input
        market_data = get_realtime_market_data(user_prompt)
        
        realtime_context = ""
        if market_data["success"]:
            realtime_context = f"""
[REAL-TIME MARKET DATA FETCHED SUCCESSFULLY]:
...
"""
            ...
            realtime_context = f"[REAL-TIME DATA FETCH NOTE]: లైవ్ డేటా ఫెచ్ కాలేదు. కారణం: {market_data['error']}\n"
        ...
        with st.chat_message("assistant"):
            with st.spinner("స్మార్ట్ మనీ కాన్సెప్ట్స్ మరియు లైవ్ డేటాతో విశ్లేషిస్తోంది..."):
                try:
                    ...
                    full_prompt = f"{realtime_context}\n[User Context: Market={market_focus}, Timeframe={timeframe}]\nయూజర్ ప్రశ్న: {user_prompt}"

                    if uploaded_image:
                        response = model.generate_content([full_prompt, uploaded_image])
                    else:
